...
func (r *runners) InitCustomersArchiveCommand(parent *cobra.Command) *cobra.Command {
	...
	cmd.Flags().StringVar(&r.args.archiveCustomerNameOrId, "customer", "", "The Customer Name or ID")

	return cmd
}

func (r *runners) archiveCustomer(_ *cobra.Command, _ []string) error {
	if r.args.archiveCustomerNameOrId == "" {
	...
	if err := r.api.ArchiveCustomer(r.appType, customer.ID); err != nil {
		return errors.Wrapf(err, "archive customer %q", r.args.archiveCustomerNameOrId)
	}
