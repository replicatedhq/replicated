...
func (r *runners) InitCollectorCreate(parent *cobra.Command) {
	cmd := &cobra.Command{
		...
		Long:  `Create a new collector by providing YAML configuration`,
	}
	...
	cmd.Flags().StringVar(&r.args.createCollectorYamlFile, "yaml-file", "", "The file name with YAML config for this release.  Cannot be used with the `yaml` flag.")
...
func (r *runners) collectorCreate(cmd *cobra.Command, args []string) error {
	...
	collector, err := r.api.CreateCollector(r.appID, r.args.createCollectorName, r.args.createCollectorYaml)
	if err != nil {
		return err
	}

	if _, err := fmt.Fprintf(r.w, "NAME: %s\n", collector.Name); err != nil {
		return err
	}
