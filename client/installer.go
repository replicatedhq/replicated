...
func (c *Client) CreateInstaller(appId string, appType string, yaml string) (*types.InstallerSpec, error) {
	if appType == "platform" {
		return nil, errors.Errorf("Kubernetes Installlers are not supported for platfrom appliations")
...
func (c *Client) PromoteInstaller(appId string, appType string, sequence int64, channelID string) error {
	if appType == "platform" {
		return errors.Errorf("Kubernetes Installlers are not supported for platfrom appliations")
	} else if appType == "ship" {
		return errors.Errorf("Kubernetes Installlers are not supported for platfrom appliations")
	} else if appType == "kots" {
		return c.KotsClient.PromoteInstaller(appId, sequence, channelID)
	}
