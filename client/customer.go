...
func (c *Client) CreateCustomer(appID, appType string, name string, channelID string, expiresIn time.Duration, airgap, gitops bool) (*types.Customer, error) {
	if appType == "platform" {
		...
		return c.KotsClient.CreateCustomer(name, appID, channelID, expiresIn, airgap, gitops)
	}
